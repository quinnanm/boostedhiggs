...
class HwwProcessor_nocuts(processor.ProcessorABC):
...
#         )
        ...
        for ch in self._channels:
            ...
            out["metfilters"] = pad_val(metfilters, -1)

            out["leptonKin"] = pad_val(candidatelep.pt, -1)
            out['oneMuon'] = pad_val((n_good_muons == 1) & (n_good_electrons == 0) & (n_loose_electrons == 0) & ~ak.any(loose_muons & ~good_muons, 1), -1)
            out['oneElectron'] = pad_val((n_good_muons == 0) & (n_loose_muons == 0) & (n_good_electrons == 1) & ~ak.any(loose_electrons & ~good_electrons, 1), -1)
            out["notaus_mu"] = pad_val((n_loose_taus_mu == 0), -1)
            ...
            out["oneFatjet"] = pad_val((n_fatjets >= 1) &
                                       (n_good_muons == 0) & (n_loose_muons == 0) &
                                       (n_good_electrons == 0) & (n_loose_electrons == 0), -1)
            out["fatjetKin"] = pad_val(leadingfj.pt, -1)
            out["fatjetSoftdrop"] = pad_val((leadingfj.msoftdrop > 200), -1)
