...
class HttProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        # match HToTauTau semi-lep dataset
        if "HToTauTau" in dataset:
            htt_flavor,htt_matched,matchedH,genH = match_Htt(events.GenPart,candidatefj)
            matchedH_pt = ak.firsts(matchedH.pt)
            genH_pt = ak.firsts(genH.pt)
        else:
            htt_flavor = ak.zeros_like(candidatefj.pt) 
            htt_matched = ak.zeros_like(candidatefj.pt)
