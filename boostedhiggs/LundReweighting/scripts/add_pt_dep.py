...
for h in [h_nom, h_up, h_down]:
    ...
    if h is h_up:
        func_name_base += "sys_tot_up_"
    if h is h_down:
        func_name_base += "sys_tot_down_"
    ...
    for j in range(1, h.GetNbinsY() + 1):
        for k in range(1, h.GetNbinsZ() + 1):
            ...
            for i in range(1, h.GetNbinsX() + 1):
                ...
                xmin = transform(max(center - width / 2.0, 5.0))
                xmax = transform(center + width / 2.0)
                x_err_up = abs(x_val - xmax)
                x_err_down = abs(x_val - xmin)

                if xmin > xmax:
                    x_err_up, x_err_down = x_err_down, x_err_up
            ...
            if len(x) > 1:
                ...
                while True:
                    ...
                    F = F_num / (F_denom + eps)
                    F_prob = 1.0 - ROOT.TMath.FDistI(F, 1, nbin - order)
                ...
                if write_out:
                    ...
                    g.GetXaxis().SetRangeUser(0.0, 0.1)
                    g.Draw("AP")
                    func.Draw("same")
                    c.Print(fout)
