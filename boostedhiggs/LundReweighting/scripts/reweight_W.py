...
n_pt_bins = 6
num_excjets = 2
pt_bins = array("f", [0.0, 50.0, 100.0, 175.0, 250.0, 350.0, 99999.0])
...
d_ttbar_w_match.norm_uncertainty = 0.0
d_ttbar_t_match.norm_uncertainty = 0.0
d_ttbar_nomatch.norm_uncertainty = 0.0
...
if do_sys_variations:
    ...
    for sys in keys:
        ...
        if sys in sig_sys:
            sig_sys_variations[sys] = h_mc.Clone(h_mc.GetName().replace("nom", sys))
...
if do_plot:
    for l in obs:
        ...
        if l == "mSoftDrop":
            ...
            h_range = (pt_cut, 800.0)
            n_bins_ = n_bins
        else:
            n_bins_ = n_bins
            h_range = None
        make_multi_sum_ratio_histogram(
            data=getattr(d_data, l),
            entries=a,
            weights=weights_nom,
            labels=labels,
            h_range=h_range,
...
for d in sigs:
    d.subjets = d.fill_LP(
        LP_rw, h_mc, num_excjets=num_excjets, sys_variations=sig_sys_variations, prefix=CA_prefix, rescale_subjets="vec"
    )

for d in bkgs:
    d.subjets = d.fill_LP(
        LP_rw, h_bkg, num_excjets=num_excjets, sys_variations=bkg_sys_variations, prefix=CA_prefix, rescale_subjets="vec"
    )
...
for d in [d_data] + sigs + bkgs:
    d.subjet_pt = []
    if d is d_data:
        h_subjets = h_data_subjets
    elif d in sigs:
        h_subjets = h_mc_subjets
    elif d in bkgs:
        h_subjets = h_bkg_subjets
...
nom_ratio = LP_rw.make_LP_ratio(
    h_data, h_bkg, h_mc, h_data_subjets, h_bkg_subjets, h_mc_subjets, pt_bins=pt_bins, outdir=outdir, save_plots=True
)
...
if do_sys_variations:
    ...
    for i, sys_name in enumerate(keys):
        ...
        for d in bkgs:
            if sys_name == "bkg_norm_up":
                weights_sys = d.get_weights() * (1.0 + d.norm_unc)
            elif sys_name == "bkg_norm_down":
                weights_sys = d.get_weights() * (1.0 - d.norm_unc)
            else:
        ...
        sys_ratio = LP_rw.make_LP_ratio(
            h_data, h_bkg_sys, h_mc_sys, h_data_subjets, h_bkg_subjets_sys, h_mc_subjets_sys, pt_bins=pt_bins
        )
