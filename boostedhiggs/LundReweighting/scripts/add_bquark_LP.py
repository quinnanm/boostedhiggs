...
for i, pf_cand_top in enumerate(pf_cands_top):
    top_subjets[i], splittings[i] = rw.get_splittings(
        pf_cand_top, num_excjets=3, rescale_subjets="vec", rescale_val=top_pt[i]
    )
...
for i in range(len(top_subjets)):
    if top_qs_samejet[i] or top_all_far[i]:
        continue
...
for i in range(1, h_qs.GetNbinsX() + 1):
    ...
    h_qs_proj.Scale(1.0 / h_qs_proj.Integral())
    h_qs_W_proj.Scale(1.0 / h_qs_W_proj.Integral())
    h_bs_proj.Scale(1.0 / h_bs_proj.Integral())
    ...
    cleanup_ratio(h_ratio2_proj, h_min=0.0, h_max=2.0)
