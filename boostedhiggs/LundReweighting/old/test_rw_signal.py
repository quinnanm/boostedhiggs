...
weights_nom = np.array([1.0] * len(tau21_sig))
...
LP_weights, LP_uncs, LP_smeared_weights = d_sig.reweight_LP(
    h_ratio,
    subjet_rw=subjet_rw,
    fill_z=fill_z,
    jetR=jetR,
...
if do_sys_variations:
    for sys in sys_weights_map.keys():
        ...
        sys_LP_weights, _ = d_sig.reweight_LP(
            sys_ratio,
            subjet_rw=subjet_rw,
            fill_z=fill_z,
            jetR=jetR,
...
for _, sys_weights in sys_variations.items():
    ...
    LP_weights_overall_unc += (diff_weights / 2.0) ** 2

LP_weights_overall_unc = np.sqrt(LP_weights_overall_unc)
print(LP_weights_stat_unc[:5])
print(LP_weights_overall_unc[:5])

LP_weights_up = np.clip(LP_weights + LP_weights_overall_unc, 0.0, 10.0)
LP_weights_down = np.clip(LP_weights - LP_weights_overall_unc, 0.0, 10.0)
print(LP_weights[:10])
...
make_histogram(
    ...
    "b",
    "Weight Statistical Fractional Uncertainty ",
    "Lund Plane Reweighting Factors Uncertainty",
    20,
    normalize=False,
    fname=outdir + "lundPlane_weights_unc.png",
    h_range=(0.0, 1.5),
)
...
make_multi_ratio_histogram(
    ...
    colors,
    "Tau21",
    "Tau21 : Before vs. After Lund Plane Reweighting",
    n_bins,
...
make_multi_ratio_histogram(
    ...
    colors,
    "tau32",
    "tau32 : Before vs. After Lund Plane Reweighting",
    n_bins,
    ratio_range=ratio_range,
    normalize=True,
    weights=weights,
...
make_multi_ratio_histogram(
    ...
    colors,
    "tau43",
    "tau43 : Before vs. After Lund Plane Reweighting",
    n_bins,
    ratio_range=ratio_range,
    normalize=True,
    weights=weights,
...
make_multi_ratio_histogram(
    ...
    colors,
    "nPF",
    "nPF : Before vs. After Lund Plane Reweighting",
    n_bins,
    ratio_range=ratio_range,
    normalize=True,
    weights=weights,
...
make_multi_ratio_histogram(
    ...
    colors,
    "mSoftDrop",
    "mSoftDrop : Before vs. After Lund Plane Reweighting",
    n_bins,
    ratio_range=ratio_range,
    normalize=True,
    weights=weights,
    fname=outdir + "msd_before_vs_after.png",
)
