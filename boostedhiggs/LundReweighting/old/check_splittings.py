...
top_msd_cut_mask = (top_jet_kinematics[:, 3] * jms_corr > m_cut_top_min) & (
    top_jet_kinematics[:, 3] * jms_corr < m_cut_top_max
)
...
print("\nW-subjet matching :")
print(
    "Q1 dists, avg %.3f, std %.3f. Frac over %.1f %.3f"
    % (np.mean(w_q1_close), np.std(w_q1_close), deltaR_cut, np.mean(w_q1_close > deltaR_cut))
)
print(
    "Q2 dists, avg %.3f, std %.3f. Frac over %.1f %.3f"
    % (np.mean(w_q2_close), np.std(w_q2_close), deltaR_cut, np.mean(w_q2_close > deltaR_cut))
)
...
print("Frac with pt > 350 : %.3f" % np.mean(w_subjets_pt > 350.0))
make_histogram(
    w_subjets_pt,
    "W subjets",
    colors="blue",
    xaxis_label="Subjet pt (GeV)",
    title="W-jets : subjet pt",
    num_bins=40,
    normalize=True,
    fname=outdir + "W_subjet_pt.png",
)


make_histogram(
    [w_q1_close, w_q2_close],
    ["Q1", "Q2"],
    colors=["red", "blue"],
    xaxis_label=r"$\Delta R$ to closest sub-jet",
    title="W-jets : quark-subjet matching %s" % jet_str,
    num_bins=40,
    normalize=True,
    fname=outdir + "W_quark_%s_matching.png" % jet_str,
)
...
print(
    "Q1 dists, avg %.3f, std %.3f. Frac over %.1f %.3f"
    % (np.mean(top_q1_close), np.std(top_q1_close), deltaR_cut, np.mean(top_q1_close > deltaR_cut))
)
print(
    "Q2 dists, avg %.3f, std %.3f. Frac over %.1f %.3f"
    % (np.mean(top_q2_close), np.std(top_q2_close), deltaR_cut, np.mean(top_q2_close > deltaR_cut))
)
print(
    "b dists, avg %.3f, std %.3f. Frac over %.1f %.3f"
    % (np.mean(top_b_close), np.std(top_b_close), deltaR_cut, np.mean(top_b_close > deltaR_cut))
)
...
make_histogram(
    [top_q1_close, top_q2_close, top_b_close],
    ["Q1", "Q2", "b Quark"],
    colors=["red", "blue", "green"],
    xaxis_label=r"$\Delta R$ to closest sub-jet",
    title="top-jets : quark-subjet matching %s" % jet_str,
    num_bins=40,
    normalize=True,
    fname=outdir + "top_quark_%s_matching.png" % jet_str,
)

top_subjets_pt = top_subjets[:, :, 0].reshape(-1)
print("Frac with pt > 350 : %.3f" % np.mean(top_subjets_pt > 350.0))
make_histogram(
    top_subjets_pt,
    "top subjets",
    colors="blue",
    xaxis_label="Subjet pt (GeV)",
    title="top-jets : subjet pt",
    num_bins=40,
    normalize=True,
    fname=outdir + "top_subjet_pt.png",
)
