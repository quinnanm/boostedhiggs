...
f_sig = h5py.File(
    "/uscms_data/d3/oamram/CASE_analysis/src/CASE/LundReweighting/CASE_signals/ZpToTp_Zp5000_Tp400_test.h5", "r"
)
label = "ZpToTpTp"
n_prongs = (5, 5)
sig_mass = 5000.0
...
if label == "Radion":
    cut = cut & (j1_m > 70.0) & ((j2_m > 70.0) & (j2_m < 100.0))
...
n_repeated = [
    np.sum(j_closest[i] < deltaR_cut) - np.unique(j_which[i, j_closest[i] < deltaR_cut]).shape[0]
    for i in range(j_which.shape[0])
]
...
make_histogram(
    ...
    title="%s : quark-subjet matching (%i prongs)" % (label, n_prongs[j_idx]),
    num_bins=40,
    normalize=True,
    fname=options.outdir + label + "_j%i_quark_subjet_matching.png" % j_idx,
)
...
make_histogram(
    ...
    title="%s : subjet pt (%i prongs)" % (label, n_prongs[j_idx]),
    num_bins=40,
    normalize=True,
    fname=options.outdir + label + "_j%i_subjet_pt.png" % j_idx,
)

print("Fraction of subjets with pt > 350 : %.3f" % (np.mean(j_subjet_pts > 350.0)))
print("Pt scaling unc would be +/- %.3f" % (np.mean(j_subjet_pts > 350.0) * 0.21 * (n_prongs[j_idx])))
...
make_histogram(
    ...
    h_range=(0.5, 16.5),
    normalize=True,
    fname=options.outdir + label + "_decay_pdgid.png",
)
