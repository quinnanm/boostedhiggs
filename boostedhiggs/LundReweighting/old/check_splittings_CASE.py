...
f_sig = h5py.File(
    "/uscms_data/d3/oamram/CASE_analysis/src/CASE/LundReweighting/CASE_signals/ZpToTp_Zp5000_Tp400_new.h5", "r"
)
label = "ZpToTpTp"
n_prongs = (5, 5)
sig_mass = 5000.0
...
n_repeated = [
    np.sum(j_closest[i] < deltaR_cut) - np.unique(j_comb_which[i, j_closest[i] < deltaR_cut]).shape[0]
    for i in range(j_comb_which.shape[0])
]
...
print("\nSubjet matching :")
print(
    "Overall avg %.3f, std %.3f. Frac of subjets over %.1f %.3f, frac of events %.3f"
...
make_histogram(
    ...
    title="%s : quark-subjet matching (%i+%i prongs)" % (label, n_prongs[0], n_prongs[1]),
    num_bins=40,
    normalize=True,
    fname=outdir + label + "_quark_subjet_matching.png",
)

j1_subjet_pts = j1_subjets[:, :, 0].reshape(-1)
j2_subjet_pts = j2_subjets[:, :, 0].reshape(-1)
make_histogram(
    [j1_subjet_pts, j2_subjet_pts],
    ["J1 Subjets", "J2 Subjets"],
    colors=["blue", "red"],
    xaxis_label="Subjet pt (GeV)",
    title="%s : subjet pt (%i+%i prongs)" % (label, n_prongs[0], n_prongs[1]),
    num_bins=40,
    normalize=True,
    fname=outdir + label + "_subjet_pt.png",
)

all_subjet_pts = np.append(j1_subjet_pts, j2_subjet_pts)
print("Fraction of subjets with pt > 350 : %.3f" % (np.mean(all_subjet_pts > 350.0)))
print("Pt scaling unc would be +/- %.3f" % (np.mean(all_subjet_pts > 350.0) * 0.21 * (n_prongs[0] + n_prongs[1]) / 2))
...
make_histogram(
    ...
    "",
    colors="blue",
    xaxis_label="Decay Product PdgID",
    title="%s : decay products (%i+%i prongs)" % (label, n_prongs[0], n_prongs[1]),
    num_bins=16,
    h_range=(0.5, 16.5),
    normalize=True,
    fname=outdir + label + "_decay_pdgid.png",
)
