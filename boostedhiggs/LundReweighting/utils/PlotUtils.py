...
def plot_training(hist, fname=""):
    ...
    for label, loss_hist in hist.items():
        if len(loss_hist) > 0:
            plt.plot(epochs, loss_hist, colors[idx], label=label)
        idx += 1
...
def make_histogram(
    ...
    ns, bins, patches = plt.hist(
        entries, bins=num_bins, range=h_range, color=colors, alpha=alpha, label=labels, density=normalize, histtype=h_type
    )
...
def make_outline_hist(
    ...
    if n_stacks > 0:
        plt.hist(
            ...
            bins=num_bins,
            range=h_range,
            color=colors[:n_stacks],
            alpha=0.2,
            label=labels[:n_stacks],
            density=normalize,
            histtype="barstacked",
        )
    if len(outlines) > 0:
...
def make_multi_ratio_histogram(
    entries,
    labels,
    colors,
    axis_label,
    ...
    ns, bins, patches = ax0.hist(
        entries,
        bins=num_bins,
        range=h_range,
        color=colors,
        alpha=alpha,
        label=labels[: len(entries)],
        density=normalize,
        weights=weights,
        histtype=h_type,
    )
    ...
    if unc_band_norm > 0:
        ...
        if not normalize:
            norm_dist = raw_dist * unc_band_norm / np.sum(raw_dist)
        else:
            normed_dist = raw_dist * bin_size * unc_band_norm
        # print(raw_dist)
        ...
        if len(labels) <= len(entries):
            label = "Signal Injection Stat. Unc."
        else:
            label = labels[-1]

        l_unc = ax1.fill_between(bincenters_band, 1.0 - frac_unc, 1.0 + frac_unc, color="gray", alpha=0.5, label=labels)
    ...
    for i in range(1, len(ns)):
        ...
        ax1.errorbar(
            bincenters, ratio, yerr=ratio_err, alpha=alpha, markerfacecolor=colors[i], markeredgecolor=colors[i], fmt="ko"
        )
            ...
            # Otherwise...
                ...
                if stack:
                    ...
                    for bkg_index, bkg in enumerate(bkglist[hist_index]):  # Won't loop if bkglist is empty
                        ...
                        if bkgNames == []:
                            this_bkg_name = bkg.GetName().split("_")[0]
                        elif type(bkgNames[0]) != list:
                            this_bkg_name = bkgNames[bkg_index]
                        else:
                            this_bkg_name = bkgNames[hist_index][bkg_index]
                ...
                if len(signals) > 0:
                    ...
                    if signalNames == []:
                        this_sig_name = signals[hist_index].GetName().split("_")[0]
                ...
                r_axis_hist.GetXaxis().SetTitleOffset(1.0)
...
def makeRatio(DATA, BKG):
    ...
    x_err_low = array("d")
    x_err_high = array("d")
    x_err_zero = array("d", [0.0] * nbins)
    y_val1 = array("d", [1.0] * nbins)
...
def get_eff_unc(data=None, weight=None, name="h", num_bins=1000, unc=None, cut_val=0.0):
...
def make_multi_sum_ratio_histogram(
    data=None,
    entries=None,
    labels=None,
    uncs=None,
    ...
    for i, e in enumerate(entries):
        ...
        if uncs is not None:
            unc = uncs[i]
        hist = make_root_hist(
            data=e, weight=weight, unc=unc, name="h" + labels[i], num_bins=num_bins, bin_low=low, bin_high=high
        )
        if stack:
...
def make_ratio_histogram(
    ...
    if max_rw > 0:
        ratio = np.clip(ratio, 1.0 / (2 * max_rw), max_rw)
