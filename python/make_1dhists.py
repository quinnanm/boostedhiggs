        ...
        for ch in channels:
            for sample in samples[year][ch]:
                ...
                for i, parquet_file in enumerate(parquet_files):
                    ...
                    num_events['preselection'] = num_events['preselection'] + len(data[var[0]])
                    num_events['btag'] = num_events['btag'] + len(data[var[0]][data["anti_bjettag"] == 1])
                    num_events['dr'] = num_events['dr'] + len(data[var[0]][data["leptonInJet"] == 1])
                    num_events['btagdr'] = num_events['btagdr'] + len(data[var[0]][data["anti_bjettag"] == 1][data["leptonInJet"] == 1])
