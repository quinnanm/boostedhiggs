...
def make_stacked_hists(year, ch, idir, odir, samples):
    ...
    hists = hist2.Hist(
        sample_axis,
        axis_dict['fj_pt'],     # will be used to order the samples in the histogram
    ...
    with open(f"{odir}/{ch}_hists_regions.pkl", "wb") as f:
        pkl.dump(hists, f)
...
def plot_stacked_hists(year, ch, odir, logy=True, add_data=True, add_soverb=True):
    ...
    # load the hists
    with open(f"{odir}/{ch}_hists_regions.pkl", "rb") as f:
        hists = pkl.load(f)
        f.close()

    # make the histogram plots in this directory
    if logy:
        if not os.path.exists(f"{odir}/{ch}_hists_regions_log"):
            os.makedirs(f"{odir}/{ch}_hists_regions_log")
    else:
        if not os.path.exists(f"{odir}/{ch}_hists_regions"):
            os.makedirs(f"{odir}/{ch}_hists_regions")
    ...
    # regions
    regions = {}
    regions['tagger_score'] = [0j, 0.8j, 1j]    # regions to explore
    ...
    all_regions = []    # will be a list of tuples
    for tagger_score_bin in tuple_tagger_score:
        all_regions.append(hists[{'tagger_score': slice(tagger_score_bin[0], tagger_score_bin[1])}][{'tagger_score': sum}])

    # for numb, region in enumerate(all_regions):
    #     all_regions[numb] = region[{'tagger_score': sum}]

    for numb, region in enumerate(all_regions):

        # get histograms

        order = region[{'met_over_pt': sum}]
        h = region[{'var': sum}]
        ...
        for bkg_label in bkg_labels:
            order_dic[simplified_labels[bkg_label]] = order[{"samples": bkg_label}].sum()
        ...
        if sax is not None:
            ...
            sax.set_xlabel(f"MET/Jet pt [GeV]")
            rax.set_ylabel("Data/MC", fontsize=20)

        elif rax is not None:
            ax.set_xlabel("")
            rax.set_xlabel(f"MET/Jet pt [GeV]")
            rax.set_ylabel("Data/MC", fontsize=20)
        ...
        if logy:
            plt.savefig(f"{odir}/{ch}_hists_regions_log/met_over_fj_pt_{numb}.pdf", bbox_inches="tight")
        else:
            plt.savefig(f"{odir}/{ch}_hists_regions/met_over_fj_pt_{numb}.pdf", bbox_inches="tight")
        plt.close()
...
def main(args):
    ...
    for ch in channels:
        if args.make_hists:
            if len(glob.glob(f"{odir}/{ch}_hists_regions.pkl")) > 0:
...
if __name__ == "__main__":
    # e.g.
    # run locally as: python make_stacked_hists_met_over_lep.py --year 2017 --odir hists --channels ele --idir /eos/uscms/store/user/cmantill/boostedhiggs/Sep2 --plot_hists --make_hists
